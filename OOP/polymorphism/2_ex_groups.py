...
class Group:
    ...
    def __str__(self):
        ...
        output += ', '.join([f"{p.name} {p.surname}" for p in self.people])
        return output

    def __getitem__(self, index):
        searched_person = self.people[index]
        return f"Person {index}: {searched_person.name} {searched_person.surname}"
